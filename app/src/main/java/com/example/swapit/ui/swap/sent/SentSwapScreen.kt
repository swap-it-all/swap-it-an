...
import com.example.swapit.ui.swap.received.ReceivedSwapScreenCard
import com.example.swapit.ui.swap.received.ReceivedSwapTopBar
import com.example.swapit.ui.theme.Paddings
import com.example.swapit.ui.theme.Typography

@Composable
fun SentSwapScreen(navController: NavHostController, sentSwaps: List<SentSwap>) {
    Scaffold(topBar = { SentSwapTopBar(navController = navController) }) { contentPadding ->
        LazyColumn(
            modifier = Modifier.padding(contentPadding)
        ) {
            items(sentSwaps.size,
                key = { index -> sentSwaps[index].tradesId }) { index ->
